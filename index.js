...
app.get('/mis-guilds/:userId', async (req, res) => {
  ...
  try {
    ...
    <div class="wrap">
      <header><div><h1>Servidores donde eres Owner</h1><div style="opacity:.8">Accede al panel para moderación, comandos y logs</div></div><div><a class="btn" href="/login">Cambiar cuenta</a></div></header>
      <section class="grid">${guildsHtml}</section>
      <p style="opacity:.8;margin-top:14px">Si no ves un servidor, verifica que Abyssus esté invitado y que tu cuenta sea la dueña del servidor.</p>
    </div></body></html>`);
  } catch (err) {
    ...
    <div class="wrap">
      ...
      <div class="main">
        ...
        <div class="panel">
          ...
          <div style="display:flex;gap:8px"><button class="primary" onclick="sendMessage()">Enviar</button><button onclick="document.getElementById('messageContent').value='!ayuda'">Comando: !ayuda</button></div>
